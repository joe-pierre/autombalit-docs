...
- **Purge de `PositionCamion` (rétention 24-48h) jamais implémentée.** `SPEC.md` §3 (« Politique de rétention ») déclare l'historique brut `PositionCamion` conservé 24-48h maximum puis remplacé par des agrégats anonymisés ; `DECISIONS.md` (entrée « Rétention des positions ») en fait un **🔵 Choix assumé** depuis la conception initiale. Vérifié (recherche exhaustive dans `autombalit-backend` — aucune occurrence de `purge`, aucune tâche planifiée `celery`/`crontab`, aucun service de ce type dans `docker-compose.yml`, seules les commandes de management existantes sont des commandes de seed/reset de test et `run_mqtt_listener`) : **aucun mécanisme ne supprime jamais ces positions**, la table croît sans limite depuis la mise en production de la Tâche 9. Deux entrées `DECISIONS.md` antérieures (Tâche 10, complément backend Tâche 19 — « dernier passage ») affirmaient à tort que cette purge existait et s'exécutait ; corrigées séparément pour refléter qu'il s'agit d'une politique déclarée, non d'un mécanisme en place.
  **Urgence croissante avec le pilote (Phase 5) :** tant que la Phase 5 n'a pas démarré, l'impact reste limité (peu de camions/positions de test). Une fois le camion pilote réellement en tournée au quotidien (SPEC.md §1 — fréquence de publication adaptative, 10-30s), `PositionCamion` va accumuler en continu, sans aucune borne, pour toute la durée du pilote — croissance de la base non maîtrisée, et écart croissant avec l'engagement de confidentialité annoncé côté citoyen/CDP (`SPEC.md` §9, `TODO.md` Phase 0 — « durée de conservation » de la politique de confidentialité). À implémenter avant ou dès le lancement du pilote, pas après coup : une commande de purge planifiée (management command + `cron`/Celery beat, à trancher) qui supprime les `PositionCamion` de plus de 24-48h après avoir alimenté les agrégats prévus (`SPEC.md` §3), en conservant `tracking.selectors.positions.get_dernier_passage_zone`, qui dépend aujourd'hui implicitement de cette purge pour que sa fenêtre de recherche reste bornée dans le temps sans filtre de date explicite (voir `DECISIONS.md`) — à l'inverse, `tracking.PassageZone` (outil de précision ETA) a été conçu précisément pour ne pas dépendre de cette purge, non implémentée à ce jour.

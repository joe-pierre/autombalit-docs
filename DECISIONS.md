...
**Vérification :** `flutter analyze` propre ; `flutter test` : 42 tests au vert (dont 10 nouveaux — 4 sur l'historique SQLite réel via `sqflite_common_ffi`, 6 sur `NotificationsProvider` avec mocks `mocktail` sur le service FCM/repositories : chargement de l'historique, transmission du token, absence de token non transmise, échec de transmission absorbé, notification foreground persistée en tête d'historique, tap déclenchant puis consommant la navigation). `flutter build apk --flavor citoyen --debug` réussi (mêmes avertissements Gradle/AGP/Kotlin déjà documentés Tâche 3, non bloquants). **Test interactif complet sur appareil physique (envoi réel d'une notification FCM, foreground/background, tap) non fait par Claude Code** — reste à faire manuellement par l'utilisateur, comme pour les vérifications OTP/domicile des Tâches 14/17.
**Statut :** ✅ Résolu (code + tests + build) — test interactif sur appareil physique en attente ; commit en attente d'accord (`autombalit-mobile`, et `autombalit-backend` pour le complément `/api/me/` ci-dessus).
...
**Statut :** ✅ Résolu — token FCM confirmé transmis/persisté, notification de test reçue de bout en bout sur l'appareil physique. Reste seulement l'accord explicite avant commit (`autombalit-mobile`, `autombalit-backend`).

...
**Correction (Tâche 25, branche `feat/web-admin-upload-geojson-carte`) :** `Zone.societe` (FK vers `Societe`, `null=True, blank=True`) a finalement été ajouté — l'absence de FK décidée ci-dessus n'est donc plus tout à fait exacte. Motivation : l'interface web admin d'upload/visualisation GeoJSON (Tâche 25) doit scoper les zones à la société de l'admin connecté, et attendre un rattachement via `TourneeZone` aurait empêché de voir une zone tout juste uploadée sur la carte avant toute création de `Tournee` — cassant le critère d'acceptation de cette tâche. Le champ reste nullable à dessein : `TourneeZone` demeure la source de vérité sur qui dessert réellement une zone (une zone reste un objet potentiellement partageable entre sociétés, `SPEC.md` §8) ; `Zone.societe` ne trace que la société qui a créé/uploadé la zone en premier. Décision utilisateur explicite (pas une déduction du modèle). Zone existantes non rattachées : laissées à `societe=None` (pas de valeur par défaut arbitraire) ; la `Zone Test` a été explicitement rattachée à `Société Test Pilote` pour éviter une zone de test orpheline.
...
## [RÉSOLU] Complément backend Tâche 18 : `PATCH /api/me/` pour le `fcm_token` du citoyen

**Contexte :** l'Étape 0 de la Tâche 18 (`autombalit-mobile`) devait confirmer le mécanisme de transmission du `fcm_token` de l'appareil citoyen au backend. Vérification du code réel avant de coder côté Flutter : le champ `Utilisateur.fcm_token` existe depuis la Tâche 13 (déjà utilisé en lecture par `notifications/clients/fcm_client.py`), mais aucun endpoint ne permettait de le mettre à jour — `SPEC.md` §7 prévoit `DELETE /api/me/` mais aucune route `/api/me/` n'existait dans le code. Signalé à l'utilisateur avant de coder (`AskUserQuestion`, pattern déjà anticipé par les Tâches 14/15/17 pour un cas similaire).
**Décision (endpoint) :** confirmé par l'utilisateur — nouvel endpoint dédié `PATCH /api/me/` plutôt que d'ajouter `fcm_token` en écriture sur un serializer existant sans rapport (ex. `PointEnregistreSerializer`).
**Décision (périmètre chauffeur) :** l'énoncé initial demandait que `/api/me/` gère « l'utilisateur authentifié (citoyen ou chauffeur) », mais `Chauffeur` n'a pas de champ `fcm_token` (`SPEC.md` ne prévoit aucune notification FCM côté chauffeur). Signalé à l'utilisateur avant de coder (`AskUserQuestion`) — confirmé : périmètre limité au citoyen pour cette tâche, pas d'ajout de champ `fcm_token` sur `Chauffeur` (aucun besoin identifié). Un chauffeur authentifié appelant `/api/me/` reçoit 403 (`EstCitoyen`, déjà utilisée pour `/api/points-enregistres/`, Tâche 17).
**Implémentation (dépôt `autombalit-backend`, branche `feat/api-me-fcm-token`, au-dessus de `feat/citoyen-endpoint-points-enregistres`) :**
- `api/serializers.py::FcmTokenSerializer` (`ModelSerializer` sur `Utilisateur`, `fields = ['fcm_token']`) : seul ce champ est exposé en lecture/écriture, aucun autre champ d'`Utilisateur` (`telephone`, `firebase_uid`...) n'est accessible via cet endpoint, y compris si le client en envoie un dans le payload (ignoré par le serializer, pas d'erreur de validation).
- `api/views.py::MeView` (`APIView`, `permission_classes = [EstCitoyen]`) : `patch()` construit `FcmTokenSerializer(request.user.utilisateur, data=request.data, partial=True)`, valide, sauvegarde, retourne le `fcm_token` à jour. Pas de service dédié (logique triviale, confirmé par l'utilisateur dans l'énoncé de la tâche — cohérent avec `CONVENTIONS.md`, qui réserve la couche service à la logique métier non triviale).
- `api/urls.py` : route `path('me/', MeView.as_view(), name='me')`.
**Vérification :** 5 nouveaux tests (`api/tests/test_me.py`) : mise à jour réussie et reflétée en base, tentative de modifier un autre champ (`telephone`) sans effet, absence d'effet sur le `fcm_token` d'un autre utilisateur, rejet chauffeur (403), rejet anonyme (401) ; suite complète (133 tests) au vert ; `python manage.py check` et `makemigrations --check --dry-run` propres (aucun changement de modèle, `fcm_token` existait déjà).
**Statut :** ✅ Résolu (code + tests) — commit en attente d'accord (`autombalit-backend`). Débloque la suite de la Tâche 18 côté `autombalit-mobile`.

## [RÉSOLU] Tâche 18 : app Citoyen — réception et affichage des notifications FCM

**Contexte :** dernière brique manquante côté citoyen avant l'écran statut du jour (Tâche 19). Le backend envoie déjà des notifications FCM (Tâche 13) ; le complément `PATCH /api/me/` (entrée ci-dessus) débloquait la transmission du token.
**Implémentation (dépôt `autombalit-mobile`, branche `feat/citoyen-notifications-fcm`) :**
- Packages ajoutés (versions résolues via `flutter pub add`, figées) : `firebase_messaging: ^15.2.10` (réception/permission/token) et `flutter_local_notifications: ^22.3.0` (affichage natif Android en foreground — FCM n'affiche aucune bannière tout seul quand l'app est au premier plan).
- `lib/citizen/services/fcm_notification_service.dart` (`FcmNotificationService`, couche `services/` = intégration externe, CONVENTIONS.md) : encapsule les deux SDK. `demarrer()` câble `FirebaseMessaging.onMessage` (foreground → callback + bannière locale via un canal Android dédié `seuils_eta`), `onMessageOpenedApp` et `getInitialMessage()` (tap ayant ramené l'app au premier plan, depuis l'arrière-plan ou un état tué). `gererMessageArrierePlan` (fonction top-level, `@pragma('vm:entry-point')`, enregistrée dans `main_citoyen.dart` avant `runApp`) persiste l'historique quand le message arrive dans l'isolate d'arrière-plan — Android affiche alors seul la notification système, ce handler ne fait qu'écrire en local.
- `lib/citizen/repositories/notifications_historique_repository.dart` : historique local (SQLite, `sqflite`, même pattern que `driver/position_queue_repository.dart` Tâche 16) — accessible aussi bien au premier plan que depuis l'isolate d'arrière-plan.
- `lib/citizen/repositories/fcm_token_repository.dart` : transmet le token au backend (`PATCH /api/me/`, complément ci-dessus). `ApiClient` (`lib/common/services/api_client.dart`) complété avec une méthode `patch()`.
- `lib/citizen/providers/notifications_provider.dart` (`NotificationsProvider`) : orchestre le tout — charge l'historique existant, envoie le token actuel après connexion (échec absorbé silencieusement, sans bloquer l'initialisation — un futur rafraîchissement de token ou redémarrage renverra un token à jour), écoute `onTokenRefresh`, persiste chaque notification foreground reçue, expose un signal `navigationVersHistoriqueEnAttente` consommé par `_CitizenGate`.
- `citizen_app.dart` : `_CitizenGate` devient `StatefulWidget`, initialise `NotificationsProvider` une seule fois après authentification confirmée (le token FCM n'a de sens que pour un compte connecté) ; ouvre `NotificationsHistoriqueScreen` automatiquement si l'app a été ouverte depuis une notification. `lib/citizen/screens/notifications_historique_screen.dart` (nouveau) : liste l'historique local, état vide explicite ; accessible aussi manuellement via une icône dans l'AppBar de `EnregistrementDomicileScreen`/`_ConfirmationEnregistrement`.
- `android/app/build.gradle.kts` : `isCoreLibraryDesugaringEnabled = true` + dépendance `coreLibraryDesugaring("com.android.tools:desugar_jdk_libs:2.1.4")`, requis par `flutter_local_notifications` (découvert au build, pas à l'analyse statique). `AndroidManifest.xml` : permission `POST_NOTIFICATIONS` (obligatoire à partir d'Android 13/API 33 pour afficher une notification).
**Non fait dans cette tâche** (hors périmètre annoncé) : écran statut du jour complet (Tâche 19) — le tap sur une notification ouvre l'historique local, pas un écran de statut dédié qui n'existe pas encore.
**Vérification :** `flutter analyze` propre ; `flutter test` : 42 tests au vert (dont 10 nouveaux — 4 sur l'historique SQLite réel via `sqflite_common_ffi`, 6 sur `NotificationsProvider` avec mocks `mocktail` sur le service FCM/repositories : chargement de l'historique, transmission du token, absence de token non transmise, échec de transmission absorbé, notification foreground persistée en tête d'historique, tap déclenchant puis consommant la navigation). `flutter build apk --flavor citoyen --debug` réussi (mêmes avertissements Gradle/AGP/Kotlin déjà documentés Tâche 3, non bloquants). **Test interactif complet sur appareil physique confirmé** — voir l'entrée dédiée ci-dessous (bug de build découvert et corrigé) et sa suite (tap depuis arrière-plan et depuis un état tué, confirmés par l'utilisateur) : token transmis/persisté, notification foreground affichée, et tap ouvrant directement l'historique dans les trois états (foreground/background/tuée).
**Statut :** ✅ Résolu (code + tests + build + test interactif complet confirmé) — commité (`autombalit-mobile` `feat/citoyen-notifications-fcm` 97246be, `autombalit-backend` `feat/api-me-fcm-token` 20a8e9a).

## [RÉSOLU] Tâche 18 : test interactif sur appareil physique — bug de build (`flutter build apk` sans `-t`)

**Contexte :** vérification demandée par l'utilisateur après connexion réelle sur le téléphone (`+221771234567`, domicile enregistré) : `Utilisateur.fcm_token` restait vide en base malgré la Tâche 18 codée et testée (mocks) côté `autombalit-mobile`.

**Symptôme / Problème :** aucun `PATCH /api/me/` dans les logs backend, aucune trace `FirebaseMessaging`/permission dans le logcat de l'appareil malgré une session déjà authentifiée depuis plus de 10 minutes.

**Cause :** le `flutter build apk --flavor citoyen --debug` exécuté plus tôt (pour valider la compatibilité Gradle après ajout de `flutter_local_notifications`/`firebase_messaging`) avait été lancé **sans `-t lib/main_citoyen.dart`** — Flutter compile alors par défaut `lib/main.dart` (le compteur de démo, jamais utilisé depuis la Tâche 3, laissé tel quel dans le repo). L'APK obtenue n'était donc pas l'app citoyenne du tout ; de plus, cette APK n'avait même jamais été installée sur le téléphone, qui tournait toujours le build de la Tâche 17 (`dumpsys package` : `lastUpdateTime` antérieur au code de la Tâche 18). Confirmé par capture d'écran : l'app affichait "Flutter Demo Home Page" (compteur par défaut) après un premier `adb install`/`am start` fait avec la mauvaise APK.

**Fix :** rebuild avec `flutter build apk --flavor citoyen --debug -t lib/main_citoyen.dart`, réinstallation (`adb install -r`, données/session conservées) et relance (`adb shell am start`). Vérifié ensuite de bout en bout : `PATCH /api/me/` observé dans les logs backend (401 puis 200 — refresh JWT automatique de `ApiClient` déclenché avec succès sur le token expiré), `fcm_token` non vide en base (142 caractères, forme FCM plausible), notification de test envoyée via `notifications.clients.fcm_client.envoyer_notification` (Tâche 13, pas la console Firebase) reçue et affichée nativement sur l'appareil (`NotificationManager: notify(... channel=seuils_eta ...)`) et persistée dans l'historique SQLite local (vérifié en tirant le fichier `.db` de l'appareil via `adb shell run-as`).

**Leçon :** pour un projet multi-flavors/multi-entrypoints (`main_citoyen.dart`/`main_chauffeur.dart`, Tâche 3), toute commande `flutter build`/`flutter run`/`flutter install` doit explicitement préciser `-t lib/main_<flavor>.dart` — l'omettre ne produit pas une erreur mais compile silencieusement `lib/main.dart` (le scaffold par défaut, toujours présent dans le repo), ce qui peut faire croire à tort qu'un test de build/APK a validé l'app réelle. Vérifier systématiquement (capture d'écran ou contenu affiché) que l'app obtenue est la bonne avant d'interpréter un test manuel comme concluant.

**Suite — confirmation utilisateur sur les deux derniers cas (app en arrière-plan, app tuée) :** deux notifications de test supplémentaires envoyées (même méthode, `notifications.clients.fcm_client.envoyer_notification`), l'app d'abord mise en arrière-plan (bouton accueil) puis complètement fermée entre les deux essais. Dans les deux cas, l'utilisateur confirme : réception dans la barre de notifications Android, puis tap ouvrant l'app directement sur `NotificationsHistoriqueScreen` (`onMessageOpenedApp`/`getInitialMessage`, voir implémentation ci-dessus). Les trois états (foreground, arrière-plan, tuée) sont donc désormais vérifiés de bout en bout sur appareil physique.

**Statut :** ✅ Résolu — token FCM confirmé transmis/persisté, notifications de test reçues et tap-to-open confirmé dans les trois états (foreground/arrière-plan/tuée) sur l'appareil physique. Commité (`autombalit-mobile` `feat/citoyen-notifications-fcm` 97246be, `autombalit-backend` `feat/api-me-fcm-token` 20a8e9a).

## [RÉSOLU] Complément backend Tâche 19 : `GET /api/zones/{id}/calendrier/` et `.../dernier-passage/`

**Contexte :** l'Étape 0 de la Tâche 19 (`autombalit-mobile`) devait confirmer la source du « dernier passage connu » ; vérification du code réel avant de coder côté Flutter (pattern déjà anticipé par les Tâches 14/15/17/18) : ni l'endpoint calendrier pourtant cité comme déjà fourni par le Constat de la tâche (`SPEC.md` §7), ni aucune logique de dernier passage n'existaient dans le code (`api/urls.py`, `api/views.py`, aucun selector `tracking`/`core` correspondant). Signalé à l'utilisateur avant de coder (`AskUserQuestion`, trois questions : ajout du complément calendrier, source du dernier passage, correctif du routage domicile mobile ci-dessous) — les trois options recommandées ont été confirmées.
**Décision (calendrier) :** deux actions DRF (`@action`) sur le `ZoneViewSet` existant (même pattern que `TourneeViewSet.du_jour`/`start`/`stop`, Tâche 14) plutôt que des vues séparées :
- `core/selectors/calendriers.py::get_calendrier_zone(zone)` : agrège les `CalendrierCollecte` de toutes les `Tournee` desservant la zone (`tournee__zones=zone` — une zone peut être couverte par plusieurs tournées, SPEC.md §4.8), trié par jour puis heure.
- `api/serializers.py::CalendrierCollecteSerializer` : expose `jour_semaine`, `jour_semaine_libelle` (évite de dupliquer le mapping jour→libellé côté Flutter), `heure_debut_estimee`, `tournee_nom` (désambiguïse si plusieurs tournées desservent la zone).
- `GET /api/zones/{id}/calendrier/`, permission par défaut du `ZoneViewSet` (`IsAuthenticated`, Tâche 4) — pas de restriction `EstCitoyen`, un chauffeur peut légitimement consulter un calendrier de zone.
**Décision (dernier passage) :** option confirmée « `PositionCamion` la plus récente dans le polygone de la zone » — `tracking/selectors/positions.py::get_dernier_passage_zone(zone)` : `PositionCamion.objects.filter(point__within=zone.polygone).order_by('-horodatage').first()`. **Simplification assumée par rapport à l'option initialement formulée** : pas de filtrage additionnel sur l'assignation du jour (« camion assigné aujourd'hui à une tournée desservant la zone ») — la containment géospatiale suffit à elle seule à prouver qu'un camion est passé, et l'historique brut étant purgé après 24-48h (`SPEC.md` §3), la fenêtre reste naturellement bornée sans avoir besoin de recouper avec la date d'assignation (qui exclurait à tort un passage juste avant minuit). `GET /api/zones/{id}/dernier-passage/` — endpoint non prévu par `SPEC.md` §7, ajouté comme complément. Réponse toujours `{horodatage, camion}` (jamais `data: null` de premier niveau : `StandardJSONRenderer` traite déjà `data is None` comme un cas « pas de contenu » façon 204, distinct d'un `data` structuré dont les champs sont `null` — bug détecté et corrigé pendant l'implémentation, voir tests `api/tests/test_zones.py`).
**Vérification :** 143 tests passent (`docker compose exec backend pytest`, 8 nouveaux : agrégation multi-tournées du calendrier, isolation entre zones, sélecteur dernier-passage avec/sans position dans le polygone, les deux endpoints via le client de test DRF, authentification requise) ; `python manage.py check`/`makemigrations --check --dry-run` propres (aucun changement de modèle).
**Statut :** ✅ Résolu — implémenté et testé (`autombalit-backend`, branche `feat/api-calendrier-zone-dernier-passage`), non commité au moment de la rédaction, en attente d'accord explicite avant commit (comme toujours).

## [RÉSOLU] Tâche 19 : app Citoyen — écran statut du jour + correctif de routage du domicile déjà enregistré

**Contexte :** dernier écran citoyen avant le formulaire de signalement (Tâche 20). En vérifiant le code réel (Étape 0), un gap distinct du complément backend ci-dessus a été identifié et signalé à l'utilisateur avant de coder : `_CitizenGate` (`citizen_app.dart`) décidait de l'écran à afficher uniquement à partir de l'état **en mémoire** d'`EnregistrementDomicileProvider` (jamais rechargé depuis le backend) — après un redémarrage réel de l'app, même avec un domicile déjà enregistré lors d'une session précédente, l'écran d'enregistrement aurait réapparu à tort au lieu du nouvel écran statut du jour, ce qui aurait invalidé tout test terrain (redémarrage de l'app) du critère d'acceptation de cette tâche. Correctif confirmé par l'utilisateur (troisième question de l'`AskUserQuestion`, voir entrée backend ci-dessus).
**Implémentation (dépôt `autombalit-mobile`, branche `feat/citoyen-statut-du-jour`) :**
- `citizen/models/calendrier_entree.dart`, `citizen/models/dernier_passage.dart` : structures de données pures (CONVENTIONS.md §Architecture), miroir des serializers backend ci-dessus.
- `citizen/repositories/statut_du_jour_repository.dart` : appels réseau (`GET /zones/{id}/calendrier/`, `.../dernier-passage/`), aucun cache ici.
- `citizen/repositories/calendrier_cache_repository.dart` : cache local SQLite du calendrier (TASK_PROMPTS.md Tâche 19 point 3), même pattern que `driver/position_queue_repository.dart`/`citizen/notifications_historique_repository.dart` — `sauvegarder(zoneId, entrees)` remplace intégralement le cache d'une zone (jamais de fusion entrée par entrée). Le dernier passage n'est volontairement **pas** mis en cache (information ponctuelle sans valeur une fois périmée, contrairement au calendrier).
- `citizen/providers/statut_du_jour_provider.dart` (`StatutDuJourProvider`) : au `charger()`, retrouve le domicile via `PointsEnregistresRepository.lister()` (méthode ajoutée à ce repository, absente depuis la Tâche 17 — seule `enregistrer()` existait) — le premier point de la liste en tient lieu (un seul point enregistrable pour l'instant côté app). Si le calendrier réseau échoue, repli sur `CalendrierCacheRepository.charger()` et `horsLigne = true` ; le dernier passage est best-effort (échec absorbé silencieusement, n'affecte pas le reste de l'écran).
- **Correctif de routage (`citizen_app.dart`)** : `_CitizenGate` charge désormais `StatutDuJourProvider` au démarrage (comme `NotificationsProvider`) et route vers `StatutDuJourScreen` si `domicile != null` **ou** si le chargement a échoué (repli conservateur : en cas d'échec réseau au tout premier lancement, impossible de savoir avec certitude si un domicile existe déjà — mieux vaut montrer l'écran statut du jour, qui a son propre état d'erreur/réessayer, que de renvoyer à tort un citoyen déjà inscrit vers l'enregistrement), sinon vers `EnregistrementDomicileScreen`. Un `addListener` direct sur `EnregistrementDomicileProvider` (posé dans `initState`, retiré dans `dispose`) recharge `StatutDuJourProvider` dès qu'un domicile vient d'être enregistré dans la session en cours, pour rebasculer automatiquement vers le nouvel écran sans redémarrer l'app — préféré à un callback ajouté sur `EnregistrementDomicileProvider` pour ne pas coupler ce provider (Tâche 17) à l'existence de la Tâche 19.
- **Code couleur ETA (SPEC.md §10, point 4)** : dérivé de la plus récente notification de seuil reçue **aujourd'hui** dans `NotificationsProvider.historique` (Tâche 18). Nécessite un nouveau champ `NotificationRecue.seuilMinutes` (extrait du payload `data` FCM — `notifications.services.dispatch.notifier_seuil_franchi` l'y place déjà depuis la Tâche 13/dispatch, mais seuls `titre`/`corps` étaient persistés jusqu'ici) — migration SQLite `notifications_recues.db` v1→v2 (`ALTER TABLE ... ADD COLUMN seuil_minutes`, `NULL` pour les lignes déjà persistées). Mapping couleur assumé, dérivé directement de `SPEC.md` §10 (« rouge réservé... ETA imminent <5 min ») : seuil ≤ 5 → rouge « Camion imminent », ≤ 10 → ambre « Camion proche », sinon vert « Camion en approche » — toujours doublé du texte et de la valeur en minutes (jamais la couleur seule).
- `common/services/api_client.dart` : `get`/`post`/`patch` retournent désormais `dynamic` plutôt que `Map<String, dynamic>` — l'endpoint calendrier renvoie directement une **liste** JSON (pas un objet), que l'ancienne signature ne pouvait pas représenter. Aucun appelant existant cassé (Dart autorise l'assignation implicite d'un `dynamic` à une variable non typée).
- `citizen/models/point_enregistre.dart` : ajout de `zoneId` (optionnel, défaut `null`, pour ne pas casser les constructions `const` déjà en test depuis la Tâche 17) — nécessaire pour appeler les endpoints zone ci-dessus.
**Vérification :** `flutter analyze` propre ; `flutter test` : 62 tests au vert (20 nouveaux : provider — domicile absent/sans zone/succès/repli cache/échec partiel dernier-passage/échec total ; écran — rendu calendrier, bandeau hors-ligne, dernier passage, code couleur ETA du jour vs. d'hier, domicile sans zone ; cache SQLite réel via FFI ; persistance de `seuil_minutes`). `flutter build apk --flavor citoyen --debug -t lib/main_citoyen.dart` réussi (mêmes avertissements Gradle/AGP/Kotlin déjà documentés Tâche 3, non bloquants).
**Non fait dans cette tâche** (hors périmètre annoncé) : carte live (Tâche 22). **Test interactif complet sur appareil physique non effectué par Claude Code** (redémarrage réel de l'app pour valider le correctif de routage, coupure réseau réelle pour valider le repli cache) — reste à confirmer par l'utilisateur, comme pour les tâches précédentes.
**Statut :** ✅ Résolu (code + tests + build) — test interactif sur appareil physique en attente de confirmation par l'utilisateur ; commit en attente d'accord (`autombalit-mobile` et `autombalit-backend`, séparément comme toujours).

## [RÉSOLU] Test terrain Tâche 19 : "impossible de charger le statut du jour" — tunnel `adb reverse` inactif, message d'erreur trop générique pour diagnostiquer

**Contexte :** premier test sur l'appareil physique de l'écran statut du jour — l'app affichait "impossible de charger le statut du jour — réessayez." malgré un domicile déjà enregistré (Tâche 17) et un token FCM fonctionnel (Tâche 18), réseau prétendument normal.

**Diagnostic (avant tout correctif, conformément au processus §5 de `DIRECTIVES_CLAUDE_CODE.md`) :**
- `lib/common/config/env.dart` fixe `apiBaseUrl = 'http://localhost:8000/api'`, qui suppose `adb reverse tcp:8000 tcp:8000` actif (Option B, `GUIDE_DU_DEVELOPPEUR.md` §2.6) — mécanisme déjà en place et fonctionnel aux Tâches 14/15/17/18, inchangé ici.
- `adb reverse --list` : **vide**, aucun tunnel actif au moment du test. `docker compose ps` : `backend` `healthy`, port exposé `0.0.0.0:8000` (pas restreint à `127.0.0.1`) ; un `curl localhost:8000/api/...` depuis la machine de dev répond normalement.
- Logs backend (`docker compose logs backend`) : aucune requête `GET /api/points-enregistres/`, `.../zones/{id}/calendrier/` ou `.../dernier-passage/` en provenance du téléphone pendant le test — la requête n'a jamais atteint le serveur.
- `adb logcat` : aucune trace de l'exception réelle, `StatutDuJourProvider.charger()` l'avalant silencieusement dans un `catch` générique sans log.
- **Cause :** sur le téléphone, `localhost:8000` pointe vers son propre loopback (rien n'y écoute) en l'absence du reverse tunnel — `PointsEnregistresRepository.lister()` échoue immédiatement par connexion refusée, avant tout accès réseau réel. Domicile et token FCM restent valides car acquis lors de sessions antérieures où le tunnel était actif.
- **Root cause opérationnelle non corrigée ici** (geste manuel documenté, pas un bug de code) : relancer `adb reverse tcp:8000 tcp:8000` après chaque redémarrage/rebranchement du téléphone (déjà documenté `GUIDE_DU_DEVELOPPEUR.md` §2.6, DECISIONS.md « Test mobile local »).

**Décision (correctif de diagnosticabilité, demandé explicitement) :** distinguer dans l'app "la requête n'a jamais atteint le backend" (problème de connectivité — tunnel/Wi-Fi) d'"le backend a répondu avec une erreur" (vrai bug), pour qu'un message générique ne masque plus l'un derrière l'autre à l'avenir — ce n'était pas cosmétique, l'ambiguïté du message initial a directement retardé ce diagnostic.

**Implémentation (dépôt `autombalit-mobile`) :**
- `common/models/api_exception.dart` : nouvelle `NetworkException` — échec au niveau socket (connexion refusée, hôte injoignable, timeout), distincte d'`ApiException` (le serveur a répondu, avec une erreur).
- `common/services/api_client.dart` : les deux points d'appel réseau (`_send`, `_tenterRafraichissement`) attrapent désormais `SocketException` (`IOClient`, l'implémentation par défaut de `http.Client` hors web, la fait remonter ainsi — confirmé en lisant la doc du package `http` 1.6.0) et la retraduisent en `NetworkException`. Effet de bord corrigé au passage : un échec réseau (pas un refresh token invalide) lors du rafraîchissement JWT sur 401 ne vide plus `tokenStorage`/ne lève plus `SessionExpiredException` à tort.
- `citizen/providers/statut_du_jour_provider.dart` : le message affiché distingue désormais `NetworkException` (message de connectivité), `ApiException` (message renvoyé par le backend) et le reste (message générique de dernier recours). Nouveau champ `erreurCalendrier` : un échec du calendrier pour une cause **autre** que réseau (ex. erreur serveur) n'est plus étiqueté à tort "hors ligne" (`horsLigne`) — les deux cas replient quand même sur le cache local, mais avec un message honnête sur la cause.
- `citizen/screens/statut_du_jour_screen.dart` : bandeau ambre "Hors connexion" (inchangé) vs nouveau bandeau rouge "Calendrier indisponible pour le moment (...)" pour la cause serveur — jamais les deux confondus.
**Vérification :** `flutter analyze` propre ; `flutter test` : 70 tests au vert (8 nouveaux : `ApiClient` — traduction `SocketException`→`NetworkException` sur l'appel principal et sur le refresh sans vider la session, réponse d'erreur → `ApiException`, réponse réussie ; provider — messages différenciés par cause, `erreurCalendrier` vs `horsLigne` ; écran — bandeaux distincts, jamais les deux en même temps) ; `flutter build apk --flavor citoyen --debug -t lib/main_citoyen.dart` réussi.
**Non fait ici** : pas de log de l'exception réelle en cas d'erreur générique inattendue (`catch` final de `charger()` reste muet) — pourrait être ajouté si ce cas se reproduit sans piste claire ; pas de correctif du geste `adb reverse` lui-même (documentation déjà existante, geste manuel à refaire par l'utilisateur à chaque session).
**Statut :** ✅ Résolu (code + tests + build) — commit en attente d'accord (`autombalit-mobile`).

## [RÉSOLU] Zone de test avec un polygone réel (Ouakam) + découverte : `PointEnregistre.zone` ne se recalcule pas quand la géométrie d'une Zone change

**Contexte :** pour retester l'écran statut du jour (Tâche 19) une fois le diagnostic réseau réglé, besoin de remplacer le polygone factice de la `Zone` de test (`seed_tournee_test`, Tâche 15/16) par un polygone réel couvrant le domicile de test déjà enregistré depuis l'app citoyen (Ouakam, Dakar), sans casser la `Tournee`/`TourneeZone` déjà liées. Branche `chore/zone-test-polygone-reel` (`autombalit-backend`), au-dessus de `feat/api-calendrier-zone-dernier-passage`.

**Décision (méthode) :** nouvelle commande de management `core/management/commands/update_zone_test_polygone.py` (`<geojson_path> [--nom-zone]`) plutôt qu'une manipulation one-shot en base — cohérent avec `seed_chauffeur_test`/`seed_tournee_test`. Réutilise le pipeline de validation/import GeoJSON déjà validé (`geo_import.services.zone_import`, Tâche 5) : correspondance par `nom` exact (`Zone.objects.update_or_create`), donc la géométrie est remplacée **en place**, id préservé. Le fichier fourni (`zone_ouakam.geojson`, export Google Earth/My Maps) a `properties.name` (pas `nom`) et des coordonnées à 3 dimensions (Z=0 par point, artefact de conversion KML→GeoJSON) — la commande construit elle-même la `FeatureCollection` avec les bonnes `properties.nom` et retire la 3e coordonnée avant validation (colonne PostGIS 2D, `Zone.polygone` sans `dim=3`) ; ce nettoyage reste local à cette commande de test, pas ajouté à `geojson_validation.py` (pipeline de production, hors périmètre ici — signalé comme point de vigilance ci-dessous).

**Découverte en vérifiant l'effet réel du changement (avant de considérer la tâche terminée) :** les deux `PointEnregistre` déjà enregistrés depuis l'app citoyen (domicile de test, Tâche 17) avaient `zone=None` — leurs coordonnées ne tombaient jamais dans l'ancien rectangle factice. Remplacer la géométrie de la `Zone` ne met **pas** automatiquement à jour ces `PointEnregistre` déjà persistés : `zone` est un FK figé au moment de l'enregistrement (`citizens.services.points_enregistres.creer_point_enregistre`), jamais recalculé ensuite. Sans correctif, l'écran statut du jour serait resté bloqué sur « aucune zone connue » malgré la nouvelle géométrie.

**Décision (correctif) :** la commande recalcule désormais aussi `zone` pour **tous** les `PointEnregistre` existants après la mise à jour de la géométrie, en réutilisant `citizens.services.points_enregistres.deduire_zone` (même logique que la création, pas de logique dupliquée) — idempotent, sans effet si rien n'a changé.

**Point de vigilance signalé, non traité ici (hors périmètre de cette commande de test)** : ce même gap existe en usage réel — si un jour un admin société met à jour le polygone d'une `Zone` déjà utilisée (upload GeoJSON, `POST /api/admin/geojson/upload/`), les `PointEnregistre` des citoyens déjà enregistrés dans cette zone ne se recalculeront pas non plus automatiquement. À évaluer via `BUGS_AND_ROADMAP.md` si ça devient un besoin réel (probablement un signal Django `post_save` sur `Zone`, ou une tâche de maintenance périodique) — non implémenté maintenant, l'admin web (Phase 4) n'existe pas encore et aucune zone réelle n'est encore en production.

**Vérification (contre le vrai environnement Docker Compose local, `docker compose exec backend ...`)** :
- `python manage.py update_zone_test_polygone zone_ouakam.geojson` : `Zone Test` (id=3, inchangé) mise à jour, `polygone.valid=True`, extent cohérent avec Ouakam ; les deux `PointEnregistre` de test passent de `zone=aucune` à `zone=Zone Test`.
- Rejoué une seconde fois : aucun changement détecté (idempotent confirmé), `TourneeZone` (`Tournée Test → Zone Test`) intacte.
...
## [RÉSOLU] Tâche 21 : app `realtime` (Django Channels) — consumer WebSocket + diffusion `position.update`

**Contexte :** comble le gap identifié dès la Tâche 7 (`GUIDE_DU_DEVELOPPEUR.md` §1.8) — `channels`/`channels_redis` présents dans `requirements.txt` depuis la Tâche 1 mais jamais câblés (`CHANNEL_LAYERS`/`ASGI_APPLICATION` absents). Prérequis bloquant pour la carte live citoyen (Tâche 22). Sur une branche dédiée (`feat/realtime-websocket-position-update`, `autombalit-backend`).

**Décisions (Étape 0, confirmées par l'utilisateur avant de coder, `AskUserQuestion`) :**
- **Schéma de groupe :** `camion_<camion_id>` (pas `zone_<zone_id>`) — cohérent avec un seul camion pilote pour l'instant, `SPEC.md` §6 permettant les deux.
- **Serveur ASGI :** `daphne` ajouté en tête d'`INSTALLED_APPS` plutôt que de changer la commande du service `backend` dans `docker-compose.yml` — Channels remplace alors `manage.py runserver` par une version ASGI-aware qui sert aussi les WebSockets, sans perte de l'autoreload en dev local (approche standard recommandée par Channels pour le développement, la Phase 6 production restant inchangée avec Daphne lancé explicitement via Supervisor).
- **Diffusion et rafale :** même règle anti-rafale que le calcul ETA/notification (`SPEC.md` §4.6, `tracking.selectors.positions.est_la_plus_recente`) appliquée aussi à la diffusion WebSocket — une position qui n'est pas la plus récente du camion n'est pas diffusée, pour éviter qu'un citoyen avec la carte live ouverte voie le camion "reculer" quand une rafale de positions historiques est rejouée d'un coup (lecture littérale de `SPEC.md` §6 écartée au profit de la cohérence avec la règle métier déjà en place).

**Implémentation :**
- `autombalit_backend/settings.py` : `ASGI_APPLICATION`, `CHANNEL_LAYERS` (`channels_redis`, réutilise `REDIS_URL` existant), `daphne` en tête d'`INSTALLED_APPS`.
- `autombalit_backend/asgi.py` : `ProtocolTypeRouter` (`http` inchangé, `websocket` → `JWTAuthMiddleware(URLRouter(...))`).
- `realtime/middleware.py::JWTAuthMiddleware` : résout `scope['user']` à partir d'un JWT applicatif transmis en query string (`?token=...`, seule option au moment du handshake WebSocket — pas d'en-tête `Authorization` possible) via `rest_framework_simplejwt.tokens.AccessToken`, même jeton que `POST /api/auth/token/`. Jeton absent/invalide → `AnonymousUser`, le rejet effectif étant délégué au consumer (cohérent avec `api.permissions.EstCitoyen`).
- `realtime/consumers.py::PositionConsumer` : rejette (`close(code=4401)`) toute connexion dont l'utilisateur n'a pas de lien `Utilisateur` (donc un chauffeur authentifié est refusé, seul un citoyen peut se connecter) ; sinon rejoint le groupe `camion_<camion_id>` (id extrait de l'URL) et relaie uniquement `camion_id`/`lat`/`lng`/`horodatage` au client (`SPEC.md` §6, `CONVENTIONS.md` §Broadcasting — jamais d'autre champ). `disconnect()` retire systématiquement le canal du groupe (pas de fuite de connexion).
- `realtime/routing.py` : `websocket_urlpatterns` sur `ws/camions/<camion_id>/` — pattern **sans** `^` ni `/` de tête (`URLRouter` matche par `re.search` sur `scope['path']`, qui conserve le `/` initial ; un anchor de début de chaîne y échouerait systématiquement — piège découvert en écrivant les tests).
- `realtime/services/position_broadcast.py::diffuser_position(position)` : couche service dédiée (`CONVENTIONS.md` §Architecture — pas de logique dans la commande MQTT), applique la règle anti-rafale ci-dessus puis `channel_layer.group_send(...)`.
- `tracking/management/commands/run_mqtt_listener.py` : câblage d'un simple appel supplémentaire (`diffuser_position(position)`) à la suite de l'évaluation des seuils (Tâche 11) — toujours « aucune logique métier » dans cette commande.
- `requirements.txt` : ajout de `daphne` (et transitives `Twisted`/`autobahn`/etc., figées via `pip freeze` dans le conteneur `backend`, cohérent avec la convention déjà établie).

**Bug de compatibilité découvert et corrigé avant de considérer la tâche terminée (vérification manuelle de bout en bout, pas seulement les tests automatiques) :** `redis-py` 8.1.0 (déjà dans `requirements.txt` depuis avant cette tâche, mais jamais réellement exercé par Channels jusqu'ici — voir Écart Tâche 7 ci-dessus) a changé son défaut `socket_timeout` (`None` → 5s, `redis._defaults.DEFAULT_SOCKET_TIMEOUT`). Ce défaut entre en collision avec le `BRPOP` interne de `channels_redis` (`RedisChannelLayer.brpop_timeout = 5`, non configurable autrement qu'en modifiant la classe) : dès qu'une connexion WebSocket restait ouverte plus de quelques secondes sans nouvel événement, le client Redis levait `redis.exceptions.TimeoutError` au lieu d'attendre la réponse légitime du serveur, faisant planter le consumer (fermeture code 1011) — un scénario garanti de se produire en usage réel (un camion n'envoie pas une position toutes les secondes). Reproduit manuellement (connexion WebSocket réelle + `mosquitto_pub` réel, pas seulement le client de test Channels — dont les tests, très rapides, ne laissaient jamais l'attente Redis dépasser ce délai et masquaient donc le bug). **Fix :** `CHANNEL_LAYERS['default']['CONFIG']['hosts']` passe un dict `{'address': REDIS_URL, 'socket_timeout': None}` plutôt qu'une simple URL — `channels_redis.utils.create_pool` transmet ces kwargs additionnels à `redis.asyncio.ConnectionPool.from_url`, restaurant le comportement bloquant historique (correct ici : `BRPOP` a son propre timeout côté serveur Redis, qui n'a pas besoin d'un timeout socket client plus court).

**Test de non-régression ajouté a posteriori (branche `test/websocket-timeout-regression`, `autombalit-backend`) :** les 7 tests consumer ci-dessous, à échanges immédiats (connexion → événement → assertion), ne laissaient jamais s'écouler le délai nécessaire pour faire cycler la boucle de réception interne du channel layer (BRPOP) — une régression future du fix (retrait accidentel de `socket_timeout: None`, ou nouveau changement de défaut côté `redis-py`/`channels_redis`) serait passée inaperçue malgré une suite verte. Nouveau test `realtime/tests/test_consumers.py::test_connexion_survit_a_une_periode_idle_depassant_le_brpop_timeout` : connexion WebSocket authentifiée, `asyncio.sleep(7)` (dépasse le `brpop_timeout` de 5s avec marge), puis diffusion d'une position et vérification de sa réception — si la connexion a été tuée pendant l'attente, l'exception du consumer (`redis.exceptions.TimeoutError`) remonte à ce moment via `ApplicationCommunicator.receive_output`. **Valeur du test vérifiée activement, pas supposée** : `socket_timeout: None` retiré temporairement en local → le nouveau test échoue bien avec le `redis.exceptions.TimeoutError` exact du bug original ; fix remis en place (`git diff` vérifié vide sur `settings.py`) → le test repasse. Marqué `@pytest.mark.slow` (nouveau marqueur enregistré dans `pytest.ini`) : exclu d'un run rapide via `pytest -m "not slow"` (164 tests, ~78s), inclus dans la suite complète/CI (`pytest` sans filtre, 165 tests, ~84s — surcoût d'environ 6-7s, jugé acceptable pour la garantie apportée).

**Vérification :**
- 7 nouveaux tests (`realtime/tests/test_consumers.py`, via `channels.testing.WebsocketCommunicator` + `asgiref.sync.async_to_sync` — pas de nouvelle dépendance `pytest-asyncio`, ce pont suffit et c'est le pattern utilisé par Channels lui-même) : connexion acceptée pour un citoyen authentifié, refusée sans token / avec token invalide / pour un chauffeur, réception d'un `position.update` simulé, absence de diffusion pour une position non la plus récente, absence de fuite après déconnexion. Suite complète : 164 tests passent ; `python manage.py check` et `makemigrations --check --dry-run` propres (aucun changement de modèle, `realtime` n'en a toujours aucun).
- Vérification manuelle de bout en bout contre la vraie stack Docker Compose (pas seulement les tests) : client WebSocket réel (`websockets`, installé ponctuellement dans le conteneur pour ce test, pas ajouté à `requirements.txt`) connecté à `ws://localhost:8000/ws/camions/4/?token=<JWT citoyen de test>`, resté connecté 8s sans crash (confirmation du fix ci-dessus), puis position publiée via `mosquitto_pub` avec les identifiants du compte MQTT du camion (`camion_4`, pas le compte backend en lecture seule) reçue côté client sous la forme exacte attendue (`{"camion_id": 4, "lat": ..., "lng": ..., "horodatage": ...}`).
- `docker compose up` : les logs `backend` affichent désormais `Starting ASGI/Daphne version 4.2.3 development server` (au lieu de `Starting development server`), confirmant que `runserver` sert bien l'ASGI via Channels sans changement de commande dans `docker-compose.yml`.

**Non fait dans cette tâche** (hors périmètre annoncé) : client Flutter (Tâche 22, backend uniquement ici).

**Statut :** ✅ Résolu (code + tests + vérification manuelle de bout en bout, y compris un bug de compatibilité `redis`/`channels_redis` trouvé et corrigé) — commit en attente d'accord (`autombalit-backend`).

## [RÉSOLU] Tâche 22 : app Citoyen — carte live optionnelle (WebSocket)

**Contexte :** dernière brique de la Phase 3 (`autombalit-mobile`), consomme le WebSocket `position.update` exposé par la Tâche 21. Branche `feat/citoyen-carte-live`, au-dessus de `feat/citoyen-signalement`.

**Décisions (Étape 0, confirmées par l'utilisateur avant de coder, `AskUserQuestion`) :**
- **Camion pertinent pour le groupe WebSocket** — ambiguïté du Constat (« celui desservant la zone du domicile, ou celui du dernier passage ») : réutilisation de `dernierPassage.camionId`, déjà chargé par `StatutDuJourProvider` (`GET /api/zones/{id}/dernier-passage/`, Tâche 19/20). Aucun changement backend — cohérent avec le périmètre « mobile uniquement » de cette tâche. Limite assumée : si ce camion n'est plus en tournée active aujourd'hui, l'écran s'ouvre mais n'affiche aucune mise à jour tant qu'il ne redémarre pas une tournée (aucun endpoint « camion en_cours pour cette zone aujourd'hui » n'existe côté backend — à envisager comme tâche séparée si ça devient un problème réel).
- **Absence de camion connu** (`dernierPassage.camionId == null`) : le bouton d'accès (« Voir la carte en direct », `statut_du_jour_screen.dart`) reste toujours visible (cohérent avec « Signaler un problème ») ; l'écran s'ouvre mais affiche un état vide explicite (« Aucun camion connu pour votre zone pour le moment. ») sans jamais tenter de connexion WebSocket.

**Implémentation (`autombalit-mobile`)** :
- Dépendance ajoutée : `web_socket_channel` (3.0.3, résolue via `flutter pub add`).
- `common/config/env.dart` : nouvelle constante `wsBaseUrl` (`ws://localhost:8000/ws`), même règle que `apiBaseUrl` (IP locale ou `adb reverse`).
- `citizen/models/position_live.dart` : `PositionLive` — miroir exact du payload `position.update` (`camion_id`/`lat`/`lng`/`horodatage`, DECISIONS.md Tâche 21).
- `citizen/services/position_websocket_client.dart` (`PositionWebSocketClient`) : intégration externe encapsulée (CONVENTIONS.md §Architecture), connecteur injectable (`WebSocketConnector`) pour les tests. `connecter()` attend explicitement `channel.ready` avant de retourner le flux décodé — même principe de validation explicite que `MqttPositionClient.connecter()` côté chauffeur (Tâche 15) plutôt qu'une connexion silencieusement optimiste.
- `citizen/repositories/carte_live_repository.dart` (`CarteLiveRepository`) : construit l'URI `ws://.../camions/{id}/` avec le JWT applicatif en query string (seule option au moment du handshake WebSocket, `realtime.middleware.JWTAuthMiddleware` côté backend) et traduit chaque message en `PositionLive`.
- `citizen/providers/carte_live_provider.dart` (`CarteLiveProvider`) : `ouvrir(camionId)`/`fermer()` (jamais de connexion en arrière-plan, TASK_PROMPTS.md Tâche 22 point 1/3), reconnexion automatique par minuteur fixe (3s) tant que l'écran reste ouvert (point 4) après une coupure (flux terminé/en erreur) ou un échec de connexion initial. `fermer()` est volontairement **synchrone** (n'attend pas l'annulation de l'abonnement WebSocket, `unawaited`) — même choix que `TrackingRepository.arreter()` côté chauffeur pour la capture GPS : la déconnexion immédiate prime sur l'attente de complétion.
- `citizen/screens/carte_live_screen.dart` (`CarteLiveScreen`) : `flutter_map` + marqueur repositionné à chaque `position.update` ; connexion déclenchée dans `initState` (si `camionId` non nul), fermeture dans `dispose`. La référence au provider est capturée dans `didChangeDependencies` (pas résolue directement dans `dispose`, qui s'exécute après désactivation de l'élément — voir bug ci-dessous) et l'appel à `fermer()` est différé via `scheduleMicrotask` (voir bug ci-dessous).
- Entrée d'accès ajoutée à `statut_du_jour_screen.dart` : bouton `OutlinedButton.icon` « Voir la carte en direct », toujours visible (choix Étape 0 ci-dessus).

**Bugs trouvés et corrigés pendant les tests (avant de considérer la tâche terminée)** :
1. `context.read<CarteLiveProvider>()` appelé directement dans `dispose()` levait `FlutterError: Looking up a deactivated widget's ancestor is unsafe` lors d'un dépilement de route (l'élément est déjà désactivé à ce stade). **Fix :** référence au provider capturée dans `didChangeDependencies()` (motif recommandé par le message d'erreur Flutter lui-même) plutôt que résolue dans `dispose()`.
2. Une fois ce premier bug corrigé, `fermer()` (qui appelle `notifyListeners()`) levait `setState() or markNeedsBuild() called when widget tree was locked` — l'arbre de widgets est verrouillé pendant `BuildOwner.finalizeTree()`, phase dans laquelle `dispose()` d'un widget dépilé s'exécute. **Fix :** l'appel à `provider.fermer()` depuis `CarteLiveScreen.dispose()` est différé via `scheduleMicrotask` — le verrou est levé avant l'exécution du micro-tick suivant, sans perte de l'immédiateté requise (TASK_PROMPTS.md Tâche 22 point 3 : la déconnexion doit rester quasi immédiate, pas nécessairement dans la même frame que le `dispose()`).

**Vérification :**
- `flutter test` : 89 tests passent (dont 3 nouveaux fichiers — `carte_live_repository_test.dart` : construction de l'URI avec/sans JWT, décodage des messages, délégation de `fermer()` ; `carte_live_provider_test.dart` : connexion/réception de position, échec de connexion initial puis reconnexion automatique (`fakeAsync`), coupure après connexion puis reconnexion automatique, `fermer()` empêche toute reconnexion ultérieure ; `carte_live_screen_test.dart` : état vide sans camion connu (aucune tentative de connexion), ouverture/fermeture d'écran déclenchant bien connexion/déconnexion via `Navigator.push`/`pageBack`, conformément aux Contraintes §Tests). `flutter analyze` propre.
- `flutter build apk --flavor citoyen --debug` réussi.
- **Non fait ici** : vérification de bout en bout sur le téléphone physique (carte live affichant une position réelle pendant une tournée active, coupure Wi-Fi/USB provoquant une reconnexion visible) — nécessite l'environnement Docker Compose local actif et un camion en tournée, non exécutable depuis cet environnement. Reste à confirmer par l'utilisateur avant clôture définitive du critère d'acceptation.
...
**Décision (frontend) :** htmx plutôt que templates Django classiques — justifié par l'interactivité attendue dès les tâches suivantes (listes filtrables Tâche 26/27, carte Leaflet Tâche 25) sans SPA lourde. htmx vendorisé (`web_admin/static/web_admin/js/htmx.min.js`, v2.0.10, téléchargé une fois via unpkg puis commité — pas de CDN au runtime), cohérent avec la contrainte budget zéro/self-hostable déjà tenue pour tout le reste du projet.

**Décision (CSS) :** Tailwind CSS via le binaire CLI standalone (v4.3.3, pas de dépendance Node/npm). `scripts/build_tailwind.sh` télécharge le binaire (une fois, dans `bin/tailwindcss`, non commité — `.gitignore`) puis compile `web_admin/static/web_admin/src/input.css` (`@import "tailwindcss";`, syntaxe v4) en `web_admin/static/web_admin/css/tailwind.css` (**commité** — pas régénéré automatiquement au démarrage de `docker compose`, à relancer manuellement après modification des templates/classes utilisées). Pipeline documenté dans `GUIDE_DU_DEVELOPPEUR.md` §1.12.

**Décision (app) :** nouvelle app `web_admin` (nom proposé par le Constat de la tâche, confirmé). Structure alignée sur `CONVENTIONS.md` §Architecture : `views.py` (fines), `permissions.py` (mixin), `forms.py` (styling Tailwind du formulaire de connexion, aucune logique métier), `templates/web_admin/` (`base.html` + un template par section + `partials/` par section — upload GeoJSON, calendriers, validation chauffeurs, signalements — toutes en placeholder « à venir » à ce stade, aucune logique métier dans les templates).

**Décision (permission) :** nouveau modèle `AdminSociete` dans `core/models.py` (`OneToOneField` vers `User`, `ForeignKey` vers `Societe`) — même pattern que `Chauffeur.user`/`Utilisateur.user` (Tâche 12). « Super admin » = `User.is_superuser` natif Django (déjà utilisé par `createsuperuser`), volontairement sans ligne `AdminSociete` (voit toutes les sociétés). Sélecteur réutilisable `core/selectors/admin_societe.py` (`a_acces_admin_web`, `societes_visibles`) — `societes_visibles` retourne `Societe.objects.all()` pour un super admin, uniquement la société de l'admin sinon ; base pour le scoping des vues à données réelles des Tâches 24-27. Mixin `web_admin/permissions.py::AdminWebRequiredMixin` (combine `LoginRequiredMixin` + vérification `a_acces_admin_web`, sinon `PermissionDenied` → 403) appliqué à toutes les vues web admin.

**Décision (connexion) :** `ConnexionView` (sous-classe de `django.contrib.auth.views.LoginView`), formulaire `ConnexionForm` (sous-classe de `AuthenticationForm`, classes Tailwind sur les widgets uniquement). Le champ de connexion reste `username` (`AUTH_USER_MODEL` non modifié — changer `USERNAME_FIELD` aurait un impact global sur les comptes `citoyen-<id>`/`chauffeur-<id>` de la Tâche 12, hors périmètre de cette tâche) : par convention, l'email de l'admin est saisi comme `username` à la création du compte (pas encore de vue de création de compte admin dans cette tâche — provisioning manuel via `manage.py shell`/Django admin pour l'instant).

**Vérification :** 26 nouveaux tests (`core/tests/test_admin_societe.py` — scoping `societes_visibles`, `a_acces_admin_web` ; `web_admin/tests/test_acces.py`, paramétré sur les 5 vues — accès refusé sans authentification, 403 pour un `User` sans rôle admin, 200 pour un admin société et pour un super admin, connexion réussie/échouée, déconnexion effective), suite complète 191 tests au vert (`docker compose exec backend pytest`) ; `python manage.py check`/`makemigrations --check --dry-run` propres. Vérification manuelle de bout en bout contre la vraie stack Docker Compose (pas seulement les tests) : compte admin société jetable créé/supprimé via shell, flux `curl` complet (connexion → redirection tableau de bord → message « Connecté en tant qu'admin de Société E2E Test. », confirmant le scoping) ; CSS/JS statiques vérifiés servis (200) en conditions réelles.
